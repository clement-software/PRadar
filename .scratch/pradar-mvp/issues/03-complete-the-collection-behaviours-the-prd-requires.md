...
- [ ] Waking from sleep triggers a complete reconciliation without waiting for
  the next poll, and a long sleep produces exactly one catch-up.
- [ ] The wake source is behind an interface, so tests drive it without
  sleeping the machine.
- [ ] Creating or activating an abonnement checks that the configured analysis
...
- [ ] The check is re-evaluated when the engine or model configuration
  changes, and an abonnement blocked for that reason recovers without being
  recreated.
- [ ] Usage measurements stay local and can be exported on demand by the user.
- [ ] Tests cover wake, long sleep, an unauthorised engine, a recovered
