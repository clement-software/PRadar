...
| Automatic analysis stops after three failed attempts and exposes `unavailable`. | Infinite retries and hidden PRs. | `internal/analyse`. | Durable attempt count and terminal transition. | `TestAnalysis_ThirdFailurePublishesUnavailable`; prototype `2007e82`. |
